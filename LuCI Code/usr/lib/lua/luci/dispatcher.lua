		...
		if not util.contains(accs, user) then
			if authen then
				...
				if not user or not util.contains(accs, user) then
					...
					--Latch check
					local currentAccountId = uci.cursor():get("latch",user, "account_id")
					if currentAccountId ~= nil and currentAccountId ~= '' then
						local handle = io.popen("/root/latch.sh status " .. currentAccountId)
						local status = luci.util.trim(handle:read("*a"))
						handle:close()
						if status == 'off' then
							nixio.syslog("info", "Latch locked: " .. status)
							http.status(403, "Forbidden")
							ctx.authuser = user
							luci.template.render("sysauth", {duser="root", fuser=user})
							return
						else
							nixio.syslog("info", "latch unlocked :" .. status)
						end
